...
//
// File        : mpu6050.cpp
// Part of     : MPU6050 library using HWLIB and I2C
...
#define REGISTER_FIFO_COUNTER_H                         0x72
#define REGISTER_FIFO_COUNTER_L                         0x73
#define REGISTER_FIFO_DATA                              0x74
#define REGISTER_WHO_AM_I                               0x75

//Sensor information
#define I2C_DEFAULT_ADDRESS                             0x6
...
void sensor::setGyroConfig(uint8_t range = 0){
    if(range >= 0 && range <= 3){
        ...
        write(REGISTER_CONFIG_GYRO, read8(REGISTER_CONFIG_GYRO) & 0b11100111);
    }
};
...
void sensor::setAccelConfig(uint8_t range = 0){
    if(range >= 0 && range <= 3){
        ...
        write(REGISTER_CONFIG_ACCEL, read8(REGISTER_CONFIG_ACCEL) & 0b11100111);
    }
};
...
void sensor::setFSync(uint8_t FSync = 0){
    if(FSync >=0 && FSync <=7){
        ...
        write(REGISTER_CONFIG, read8(REGISTER_CONFIG) & 0b11000111);
    }
};
...
void sensor::setDLPF(uint8_t DLPF = 0){
    if(DLPF >= 0 && DLPF <=7){
        ...
        write(REGISTER_CONFIG, read8(REGISTER_CONFIG) & 0b11111000);
    }
}
...
void sensor::setFIFO(bool enable){
    if(enable){
        write(REGISTER_USER_CONTROL, read8(REGISTER_USER_CONTROL) | 0b01000000);
    }
    else{
        write(REGISTER_USER_CONTROL, read8(REGISTER_USER_CONTROL) & 0b10111111);
    }
}

void sensor::setFIFOGyroX(bool enable){
    if(enable){
        write(REGISTER_FIFO_ENABLE, read8(REGISTER_FIFO_ENABLE) | 0b01000000);
    }
    else{
        write(REGISTER_USER_CONTROL, read8(REGISTER_USER_CONTROL) & 0b10111111);
    }
...
void sensor::setFIFOGyroY(bool enable){
    if(enable){
        write(REGISTER_FIFO_ENABLE, read8(REGISTER_FIFO_ENABLE) | 0b00100000);
    }
    else{
        write(REGISTER_USER_CONTROL, read8(REGISTER_USER_CONTROL) & 0b11011111);
    }
}

void sensor::setFIFOGyroZ(bool enable){
    if(enable){
        write(REGISTER_FIFO_ENABLE, read8(REGISTER_FIFO_ENABLE) | 0b00010000);
    }
    else{
        write(REGISTER_USER_CONTROL, read8(REGISTER_USER_CONTROL) & 0b11101111);
    }
}
